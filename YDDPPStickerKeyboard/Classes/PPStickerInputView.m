...
- (void)changeKeyboardTo:(PPKeyboardType)toType
{
    ...
    switch (toType) {
        case PPKeyboardTypeNone:
            [self.emojiToggleButton setImage:[UIImage ydd_imageWithName:@"toggle_emoji"] forState:UIControlStateNormal];
        ...
        case PPKeyboardTypeSticker:            
            [self.emojiToggleButton setImage:[UIImage ydd_imageWithName:@"toggle_keyboard"] forState:UIControlStateNormal];
...
- (void)stickerKeyboard:(PPStickerKeyboard *)stickerKeyboard didClickEmoji:(PPEmoji *)emoji
{
    ...
    UIImage *emojiImage = [UIImage ydd_emojiImageWithName:emoji.imageName];
    if (!emojiImage) {
        return;
    }
